...
class raw:
    ...
    def get_cam_len(self):
        """get the number of frames for cams"""
    ...
    def velod(self):
        ...
        return utils.yield_images(self.velod_files, mode='I')

    def get_velod(self, idx):
        """Read image file for cam3 depth map (right) at the specified index."""
        return utils.load_image(self.velod_files[idx], mode='I')
    ...
    def _get_file_lists(self):
        ...
        # deep_data_path = os.path.join("/user/SHL/Data_temp/deep_depth",self.data_path.split(os.sep)[-1])+"_dpt_large_02"
    
        # dataset_path = "/user/SHL/Data_temp/CSW/KITTI"
        dataset_path="/workspace/data2/CSW/KITTI"
        mode = self.data_path.split(os.sep)[-3]
        file_path = os.path.join(dataset_path, mode, "2011_09_26", self.drive, 'image_c', '*.{}'.format(self.imtype))
        ...
        self.cam2d_files = sorted(glob.glob(
            # Origin
            # os.path.join(self.data_path, 'image_c', '*.{}'.format(self.imtype))
            # Mono - Depth
            os.path.join(str(file_path))
            #---------

            # os.path.join("/user/SHL/Data_temp/CSW/KITTI/train/2011_09_26/2011_09_26_drive_0001_sync/image_c/*.png")
            # os.path.join(deep_data_path,  '*.{}'.format(self.imtype))
        ))
        if len(self.cam2d_files)==0:
            print("No Test")
            file_path = os.path.join(dataset_path,"val", "2011_09_26", self.drive, 'image_c', '*.{}'.format(self.imtype))
            self.cam2d_files = sorted(glob.glob(
            os.path.join(str(file_path))
            ))
        print(f'LEN {len(self.cam2d_files)}')
        tem = cv2.imread(self.cam2d_files[0])
        print(f"SHAPE {tem.shape}")
        # print(f'NAME {self.cam2d_files[0]}')
        self.velod_files = sorted(glob.glob(
            os.path.join(self.data_path, 'image_l_F', '*.{}'.format(self.imtype))
        ))
        ...
        if self.frames is not None:
            ...
            self.cam2d_files = utils.subselect_files(
                self.cam2d_files, self.frames)
            self.velod_files = utils.subselect_files(
                self.velod_files, self.frames)
