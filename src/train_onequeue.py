...
from dataset import EvalDataset as CalibDataset_withP
from utils import count_parameters, inv_transform_vectorized, inv_transform, phi_to_transformation_matrix, phi_to_transformation_matrix_vectorized, lidar_projection
...
def train_one_epoch(model_list: list, loss_fn: Loss, opt: Optimizer, loader: DataLoader,
    ...
    for i, sample in enumerate(pbar):
        ...
        cam2_d, velo_d, gt_err, gt_err_norm, cam2, velo, T, P = sample[0].cuda(), sample[1].cuda(), sample[2], \
                                                       sample[3].cuda(), sample[4].cuda(), sample[5].cuda(), sample[6].cuda(), sample[7].cuda()
        
        # gt_err = gt_err.numpy()[0]
        T_err = phi_to_transformation_matrix(gt_err.numpy()[0])
        T_np = T[0].cpu().numpy()
...
if __name__ == '__main__':
    ...
    # create data loaders
    train_ds = CalibDataset_withP(path=args.dataset, mode='test',
    ...
    for epoch in range(trained_epochs, trained_epochs+args.epoch):
        ...
        print("Global i {}".format(global_i))
        train_loss, global_i = train_one_epoch(model_list, criteria, optimizer, train_loader, epoch, global_i, writer)
