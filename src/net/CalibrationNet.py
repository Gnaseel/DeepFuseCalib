...
def global_regression_module():
    ...
    model = nn.Sequential(
        ...
        # layer 4, spp-layer, out N, 64, 14
        SPPLayer(pyramid=(1, 4, 9), mode='max'),
    )
    return model

def output_layer(in_size):
    model = nn.Sequential(
        nn.Linear(in_size, 1024),  # (1, 128)
        nn.ReLU(),
        nn.Linear(1024, 256),  # (1, 128)
        nn.ReLU(),
        nn.Linear(256, 3)  # (1, 3)
    )
    return model
...
class CalibNet(nn.Module):
    def __init__(self):
        ...
        self.rot_output = output_layer(896)
        self.trans_output = output_layer(896)
        return
    ...
    def forward(self, disp_l, disp_c):
        ...
        # global regression, out 1, 64, 14
        regressed = self.global_regression(concat).view(bs, -1)

        # output, out 1, 3
        rot = self.rot_output(regressed)
        trans = self.trans_output(regressed)

        return rot, trans  # out (1, 3), (1, 3)


if __name__ == '__main__':
    x = torch.rand((1, 1, 352, 1216)).cuda()
    x2 = torch.rand((1, 1, 352, 1216)).cuda()
