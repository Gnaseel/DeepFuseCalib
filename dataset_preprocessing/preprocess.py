...
class KITTIDataset(Dataset):
    def __init__(self, mode: Literal['train', 'test', 'val'] = 'train',
                 database: str = '/workspace/data/KITTI',
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        uimage2 = (torch.from_numpy(image2.transpose((2, 0, 1))))
        uimage3 = (torch.from_numpy(image3.transpose((2, 0, 1))))
        ...
        return {'uimage2': uimage2,'uimage3': uimage3,'image2': image2, 'image3': image3, 'focal': focal, 'raw': raw2,
    ...
    for i, drive in enumerate(dataset.drives):
        ...
        with torch.no_grad():
            ...
            for j, sample in pbar:
                ...
                uimg2 = sample['uimage2'].cuda()
                uimg3 = sample['uimage3'].cuda()
                ...
                if method_for_cam_depth is not None:
                    # depth_c = (method_for_cam_depth(uimg2, uimg3)).astype(np.uint16)
                    # print(f'PRE SIZE OF {uimg2.shape, uimg3.shape}')
                    # print(f'PRE SIZE OF {uimg2.shape, uimg3.shape}')
                    # print(f'PRE SIZE OF {img2.shape, img3.shape}')
                    # print(f'PRE SIZE OF {img2.shape, img3.shape}')
                    depth_c = (method_for_cam_depth(uimg2, uimg3))
                    print(depth_c.dtype)
                    print(np.min(depth_c))
                    print(np.max(depth_c))
                    print(np.mean(depth_c))
                    cv2.imwrite(image_c_path.joinpath(f'{j:010d}.png').as_posix(), depth_c)
                # write to disk
                # if method_for_cam_depth is not None:
...
def arg_parser():
    ...
    parser.add_argument('--data_path', type=str, help='path to the data', required=True, default='/workspace/data/KITTI')
...
def stereo2(imgL, imgR):
    stereo = cv2.StereoSGBM_create(minDisparity=0,numDisparities = 160,
                                  blockSize = 5,
                                  P1=25*8,
                                  P2=25*32,
                                  disp12MaxDiff = 0,
    ...
    # unloader = Normalize.ToPILImage()
    l = imgL[0].cpu().numpy().transpose(1,2,0)
    r = imgR[0].cpu().numpy().transpose(1,2,0)
    l = l*255
    r = r*255
    l = l.astype(np.uint8)
    r = r.astype(np.uint8)
    
    cv2.imwrite(os.path.join("/workspace/data2","col_l.png"), l)
    cv2.imwrite(os.path.join("/workspace/data2","col_r.png"), r)
    
    l = cv2.cvtColor(l, cv2.COLOR_BGR2GRAY)
    r = cv2.cvtColor(r, cv2.COLOR_BGR2GRAY)
    
    cv2.imwrite(os.path.join("/workspace/data2","gra_l.png"), l)
    cv2.imwrite(os.path.join("/workspace/data2","gra_r.png"), r)
    
    disparity = stereo.compute(l,r)
    cv2.imwrite(os.path.join("/workspace/data2","dis_.png"), disparity)
    disparity2 = cv2.normalize(disparity, None, alpha=0, beta=255,  norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U)
    # print("------DIS ---")
    # print(disparity.dtype)
    # print(disparity2.dtype)
    # print(np.min(disparity2))
    # print(np.max(disparity2))
    # print(np.mean(disparity2))
    cv2.imwrite(os.path.join("/workspace/data2","dis_nor_.png"), disparity2)

    # print(f'SIZE OF origin {l.shape}')
    # print(f'SIZE OF disparity{disparity2.shape}')
    # time.sleep(10)
    return disparity2
...
if __name__ == '__main__':
    ...
    if not args.no_cam_depth:
        ...
        # ------------
        func_for_cam_depth: Callable = stereo2
        ...
